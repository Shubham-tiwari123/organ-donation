...
const { SAVE_PATIENT_PERSONAL_INFO, GET_PATIENT_PERSONAL_INFO, LOGIN_NAVIGATE_URL } = PATIENT_CONSTANT
const { Content } = Layout;
const { Option } = Select;


const PatientHomePage = ({account, provider}) => {
  ...
  const handleSubmit = (values) => {
    ...
    setTimeout(async () => {
      ...
      try {
        const response = await axios.post(SAVE_PATIENT_PERSONAL_INFO, data)
    ...
    const fetchData = async () => {
      ...
      if (jwtToken) {
        ...
        if (decoded && decoded.sub) {
          ...
          let URL = `${GET_PATIENT_PERSONAL_INFO}=${decoded.sub}`
          console.log("Url:", URL);
          ...
          if (status==200) {
            ...
            setUserName(data.personalInfo.firstName + " " + data.personalInfo.secondName)
            localStorage.setItem("userName", data.personalInfo.firstName + " " + data.personalInfo.secondName)
